...
async def fetch_history_async(auth_token: str):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            return {"success": False, "remaining_tokens": None, "histories": [], "error": str(e)}
...
async def save_data_parallel(history_data: dict, readcount_data: dict, 
    ...
    async def post_history():
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(BACKEND_HISTORY_URL, json=history_data, headers=header) as response:
                    if response.status == 401:
                        return {"type": "history", "status": 401, "error": "Unauthorized", "message": "You are unauthorized to access this resource."}
                    return {"type": "history", "status": response.status}
            except Exception as e:
                return {"type": "history", "status": 500, "error": str(e)}
    ...
    async def post_readcount():
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(BACKEND_DOC_READ_COUNT_URL, json=readcount_data, headers=header) as response:
                    if response.status == 401:
                        return {"type": "readcount", "status": 401, "error": "Unauthorized", "message": "You are unauthorized to access this resource."}
                    return {"type": "readcount", "status": response.status}
            except Exception as e:
                return {"type": "readcount", "status": 500, "error": str(e)}
    ...
    async def post_token():
        try:
            ...
            resp = await loop.run_in_executor(
                None, 
                lambda: used_token_store(type='chatbot', used_tokens=token_data['used_tokens'], auth_token=auth_token)
            )
            if resp.status_code == 401:
                return {"type": "token", "status": 401, "error": "Unauthorized", "message": "You are unauthorized to access this resource."}
            return {"type": "token", "status": resp.status_code}
        except Exception as e:
            return {"type": "token", "status": 500, "error": str(e)}

    results = await asyncio.gather(post_history(), post_readcount(), post_token(), return_exceptions=True)
    return results
...
async def ask_doc_bot(question: str, organization: str, auth_token: str):
    try:
        ...
        # Only fetch context in parallel, history is already fetched for auth
        ...
        # Check token limit
        if history_result.get('remaining_tokens') is not None and history_result['remaining_tokens'] < 1000:
            return JSONResponse(status_code=400, content={
                "status": "error",
                "message": "Insufficient tokens to continue the conversation."
            })
        ...
        if isinstance(json_answer, str):
            try:
                ...
                json_answer = {"answer": json_answer, "used_document": False}
        ...
        readcount_data = {}
        if json_answer.get('used_document') and context:
            for c in context:
                doc_id = c.properties.get("document_id", "")
                readcount_data[doc_id] = 1
        
        history_data = {"prompt": question, "response": json_answer['answer'], "used_tokens": used_tokens}
        token_data = {"used_tokens": used_tokens}
        
        save_results = await save_data_parallel(history_data, readcount_data, token_data, auth_token)
        save_time = asyncio.get_event_loop().time() - save_start
        print(f"✅ Parallel save completed in {save_time:.2f}s")
        
        # Handle unauthorized in save
        for result in save_results:
